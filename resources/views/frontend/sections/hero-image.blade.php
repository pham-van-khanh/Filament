...
<section class="relative overflow-hidden text-white" style="background: {{ $primary }}">
  <div class="relative mx-auto max-w-[760px] md:max-w-none">
    <div class="relative h-[var(--hero-height)] overflow-hidden" style="--hero-height: {{ $height }}">
      ...
      <div class="absolute left-5 top-5 flex items-center gap-3 text-sm font-semibold text-white/85">
        <a href="{{ route('home') }}" aria-label="Về trang chủ" class="grid h-11 w-11 place-items-center rounded-full bg-white/16 backdrop-blur">
          <x-ui-icon name="arrow-left" class="h-5 w-5" />
        </a>
        <span class="inline-flex items-center gap-2 rounded-full bg-white/16 px-4 py-2 backdrop-blur md:hidden">
          <x-ui-icon name="images" class="h-4 w-4" />
          {{ $post->media_count ?: $post->media()->count() }} ảnh
        </span>
      </div>
    </div>
    <div class="mx-auto max-w-[760px] px-5 py-6" style="background: {{ $primary }}">
      @if($post->category)
        <span class="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-sm font-semibold text-white/90">
      ...
      <p class="mt-2 text-base text-white/68">{{ $data['date_range'] ?? $post->date_range }} · {{ $post->location_name }}</p>
      @if(! empty($data['tags']))
